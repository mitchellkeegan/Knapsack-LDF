...
class base_opt_model(ABC):
    def __init__(self, opt_params):
        ...
        self.base_dir = self.opt_params['problem']
        self.opt_dir = os.path.join('Opt Results',self.model_type,self.opt_params["instance folder"])
        self.instance_dir = os.path.join('Instances',opt_params['instance folder'])

    # Overwrite as needed (E.g. for small vs large instances)
    def create_results_directory(self):
        self.results_directory = os.path.join('Opt Results',
                                              self.model_type,
...
class ml_model(ABC):
    def __init__(self, model_params,training_params,directories):
        ...
        self.instance_dir = os.path.join('Instances',directories['Instance Type'])
        self.opt_result_dir = os.path.join('Opt Results', directories['Opt Model'], directories['Instance Type'])
        self.ml_result_dir = os.path.join('ML Results', self.model_type, directories['Instance Type'])
